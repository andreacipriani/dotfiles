...
const parser = require("./mi_output_parser");
const mi_output_1 = require("./mi_output");
const LLDBEvents = require("./events");
const types_1 = require("./types");
const extractors_1 = require("./extractors");
const errors_1 = require("./errors");
const rawCommandOuputParser = require("./raw_command_output_parser");
class DebugCommand {
    /**
     ...
     */
    constructor(cmd, token, done, isRawCommand = false) {
 ...
 */
class DebugSession extends events.EventEmitter {
    /**
    ...
    parseDebbugerOutput(line) {
        ...
        let result;
        var ccmd = this.cmdQueue[0]; //FIXME ?this.cmdQueue[0] != undefined/null
        if (ccmd && ccmd.isRawCommand) { //
            if (line != "^done") {
                ...
                result = {
                    recordType: mi_output_1.RecordType.Done,
                    data: ccmd.output
                };
            }
        }
            ...
            try {
                ...
                result = {
                    recordType: mi_output_1.RecordType.AsyncNotify,
    ...
    executeCommand(command, token) {
        return new Promise((resolve, reject) => {
            this.enqueueCommand(new DebugCommand(command, token, (err, data) => { err ? reject(err) : resolve(); }));
        });
    }
    /**
     ...
     * @returns A promise that will be resolved when the command response is received.
     */
    getCommandOutput(command, token, transformOutput, isRawCommand = false) {
        return new Promise((resolve, reject) => {
    ...
    getStackFrames(options) {
        ...
        return this.getCommandOutput(fullCmd, null, (output) => {
            ...
            if (Array.isArray(data)) {
                return data.map((frame) => { return extractors_1.extractStackFrameInfo(frame); });
            }
            else {
        ...
        return this.getCommandOutput(fullCmd, null, (output) => {
            ...
            if (Array.isArray(output.threads)) {
                ...
                let threads = output.threads.map((data) => {
                    let thread = extractors_1.extractThreadInfo(data);
    ...
    getGlobalVariables() {
        let fullCmd = 'ta v -T -D1';
        return this.getCommandOutput(fullCmd, null, (output) => {
            if (output) { //FIXME 
                output = output.slice(1, output.length);
                let current = rawCommandOuputParser.parse(`(0) 0 = { ${output.join("\n")} }`);
    ...
    getStackFrameVariables(threadId, frameLevel) {
        let fullCmd = 'fr v -T -D1';
        return this.getCommandOutput(fullCmd, null, (output) => {
            if (output) { //FIXME 
                let current = rawCommandOuputParser.parse(`(0) 0 = { ${output.join("\n")} \n}`);
                return current[0].variables;
    ...
    getVariableContent(variableName, threadId, frameLevel) {
        let fullCmd = 'fr v -T -D2 ' + variableName;
        return this.getCommandOutput(fullCmd, null, (output) => {
            if (output) { //FIXME 
                let current = rawCommandOuputParser.parse(output.join("\n"));
                return current[0].variables;
            }
            else {
                return [];
